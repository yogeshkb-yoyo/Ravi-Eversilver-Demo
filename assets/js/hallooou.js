...
var formData = new FormData($('#resumeForm')[0]);
	$.ajax({
		...
		success: function(response) {
			if (response.type == 'success') {
				$("#resumeForm").before("<div class='alert alert-success' role='alert'><a href='#' class='close' data-dismiss='alert'>&times;</a>" + response.text + "</div>");
				$(ajaxform).each(function() {
					this.reset();
					$(this).find('[name="submit"]').html('<i class="fa fa-paper-plane fa-fw"></i> Send');
				}).find('.valid').each(function() {
				...
				$("#resumeForm").before("<div class='alert alert-danger' role='alert'><a href='#' class='close' data-dismiss='alert'>&times;</a>" + response.text + "</div>");
			}
		}
	});
}
...
var formData = $('#franchiseForm').serialize();
	$.ajax({
		url: "assets/php/franchise.php",
		type: "POST",
		data: formData,
		success: function(response) {
			if (response.type == 'success') {
				$("#franchiseForm").before("<div class='alert alert-success' role='alert'><a href='#' class='close' data-dismiss='alert'>&times;</a>" + response.text + "</div>");
				$(ajaxform).each(function() {
					this.reset();
					$(this).find('[name="submit"]').html('<i class="fa fa-paper-plane fa-fw"></i> Send');
				}).find('.valid').each(function() {
				...
				$("#franchiseForm").before("<div class='alert alert-danger' role='alert'><a href='#' class='close' data-dismiss='alert'>&times;</a>" + response.text + "</div>");
			}
		}
	});
}
